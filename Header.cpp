...
{
	ofstream fout("D:/”чеба/—ервисный робот/OpenCvDetection/data/cppstudio.txt", ios_base::app);
...
vector<cv_Point> RoutePlanning(cv_Point src, cv_Point dst, vector<cv_Templates> templates)
{
	...
	/*определ€ем ближайшую метку*/
	cv_Point nearestLabel = DefinitionOfNearestLabel(dst, templates);

	int direct = DirectionCheck(src, nearestLabel);/*определ€ем направление движени€*/
